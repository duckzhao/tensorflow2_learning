...
'''
'''
enbedding层一般都是放在rnn网络的第一层
tf.keras.layers.Embedding(input_dim=词汇表大小，output_dim=编码维度)   词汇表大小：编码一共要表示多少个单词，编码维度：打算用几个数字表示一个单词
这一步之后生成的编码表的大小是---（词汇表大小，编码维度）  编码维度实际上就是指几列

embedding层一般都是rnn网络的第一层，在fit时直接处理train data，它和simple rnn一样，对送入的数据维度有要求，
[送入的样本数，循环核时间展开步数]，使用np.reshape（len(x_train), xxx）  感觉时间展开步数似乎就是  “一个时间序列样本数据中 小的重复单元的个数”
'''
...
# 这里的xtrain没有做编码处理，只是单纯的给字母加了个索引数字id而已，不然字母和计算的结果数字没办法一一对应起来了，编码完了肯定就是向量了，而不是单个数字
...
# 转xtrain的维度符合enbedding fit要求，[送入样本数， 循环核时间展开步数]
# 循环核时间展开步数:输入一个字母，预测一个字母，所以是1
...
model = tf.keras.models.Sequential([
    tf.keras.layers.Embedding(input_dim=5, output_dim=2),   # 生成一个5行两列的可训练矩阵---编码矩阵
...
history = model.fit(x_train, y_train, batch_size=1, epochs=100, callbacks=[cp_callback])
...
while True:
    ...
    if word == 'esc':
        ...
        # 将数字index转为符合fit适应的二维矩阵,[送入样本数， 循环核时间展开步数]
