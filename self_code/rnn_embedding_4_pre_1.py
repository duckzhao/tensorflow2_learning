...
# print(word_to_id)
train_set_scaled = list(range(26))

x_train = []
y_train = []
# 使用for循环把数据放到xtrain和ytrain里面去
for i in range(4, 26):
...
# 转xtrain的维度符合embedding  fit要求，[送入样本数， 循环核时间展开步数]
# 循环核时间展开步数:输入4个字母，预测一个字母，所以是4
...
model = tf.keras.models.Sequential([
    tf.keras.layers.Embedding(input_dim=26, output_dim=2),  # 对26个字母进行编码
    tf.keras.layers.SimpleRNN(units=10),  # 一个循环核（层），里面有三个记忆体，只有一层即最后一层，return_sequences=False
...
# 加载历史模型
check_point_save_path = './check_point/rnn_embedding_4per1.ckpt'
...
while True:
    ...
    if word == 'esc':
        ...
        # 将word转为数字向量
        word_ = word
        word = [word_to_id[zimu] for zimu in word]
        # 将向量转为符合fit适应的二维矩阵,[送入样本数， 循环核时间展开步数]
        word = np.reshape(word, (1, 4))
        ...
        tf.print('input {} -> {}'.format(word_, input_word[result[0]]))