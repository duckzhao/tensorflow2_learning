'''
在2014年cho等人简化了LSTM结构，提出了GRU网络，GRU使记忆体ht融合了长期记忆和短期记忆，ht包含了过去信息ht-1和现在信息ht波浪号，
现在信息是过去信息ht-1，过重置门rt与当前输入xt共同决定，两个门限的取值范围也是0到1之间。
tf.keras.layers.GRU()  实际在搭建的时候，和使用simpleRnn一样
...
GRU层在fit的时候也对输入数据的维度有所要求，和simplernn一样训练集和测试集都必须np.reshape->【样本数，时间步展开步数，每个时间步的样本特征数】

用连续60天的股票价格，预测第61天的股票价格，输入序列是【60 天股票价格】---时间步展开数就是60，每个时间步的特征数是1
因为是做连续序列的数字预测，所以不用使用embedding进行编码，fit数据直接进GRU层，所以train数据需要满足特征---【样本数，时间步展开步数，每个时间步的样本特征数】
'''
# 1.
import numpy as np
...
model = tf.keras.models.Sequential([
    tf.keras.layers.GRU(units=80, return_sequences=True),  # 80个记忆体，一般最后一层才是false，前面的每层都要输出每个时间步的预测结果
    tf.keras.layers.Dropout(0.2),
    tf.keras.layers.GRU(units=100, return_sequences=False),  # 最后一层rnn了
    tf.keras.layers.Dropout(0.2),
...
# 加载历史模型
check_point_save_path = './check_point/gru_stock.ckpt'
