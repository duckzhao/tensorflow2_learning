...
SimpleRNN在fit时对送入的训练集维度是有要求的---[送入样本数， 循环核时间展开步数， 每个时间步输入特征个数]。送入样本数（len(x_train)即可），实际上就是对原始数据集做reshape
循环核时间展开步数见概念4，每个时间步输入特征个数实际上就是“一个序列中单个样本的特征数”
...
x_train = [
    ...
    [id_to_onehot[word_to_id['c']], id_to_onehot[word_to_id['d']], id_to_onehot[word_to_id['e']],
     id_to_onehot[word_to_id['a']]],
    [id_to_onehot[word_to_id['d']], id_to_onehot[word_to_id['e']], id_to_onehot[word_to_id['a']],
     id_to_onehot[word_to_id['b']]],
    [id_to_onehot[word_to_id['e']], id_to_onehot[word_to_id['a']], id_to_onehot[word_to_id['b']],
     id_to_onehot[word_to_id['c']]],
    ]
y_train = [word_to_id['e'], word_to_id['a'], word_to_id['b'], word_to_id['c'], word_to_id['d']]
...
# 循环核时间展开步数:输入4个字母，预测一个字母，所以是4---每个时间步输入特征个数：每个时间步输入序列中的一个元素，每个字母的特征数是5，所以是5
...
# 加载历史模型
check_point_save_path = './check_point/rnn_onehot_4per1.ckpt'
...
while True:
    word = input('please input four word, exit input esc!')
    if word == 'esc':
        break
    else:
        # 将word转为数字向量
        word_ = word
        word = [id_to_onehot[word_to_id[zimu]] for zimu in word]
        # 将向量转为符合fit适应的三维矩阵,[送入样本数， 循环核时间展开步数， 每个时间步输入特征个数]
        word = np.reshape(word, (1, 4, 5))
