...
因为是做连续序列的数字预测，所以不用使用embedding进行编码，fit数据直接进simpleRnn层，所以train数据需要满足特征---【样本数，时间步展开步数，每个时间步的样本特征数】
'''
# 1.
import numpy as np
...
# 将测试集修改为符合 simplernn层输入要求的维度
x_test = x_test.reshape((x_test.shape[0], 60, 1))

# 3.
model = tf.keras.models.Sequential([
    tf.keras.layers.SimpleRNN(units=80, return_sequences=True),  # 80个记忆体，一般最后一层才是false，前面的每层都要输出每个时间步的预测结果
    tf.keras.layers.Dropout(0.2),
    tf.keras.layers.SimpleRNN(units=100, return_sequences=False),  # 最后一层rnn了
    tf.keras.layers.Dropout(0.2),
...
# 加载历史模型
check_point_save_path = './check_point/simple_rnn_stock.ckpt'
